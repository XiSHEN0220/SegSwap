...
def plotCorres(IA, IB, xA, yA, xB, yB, mask, img_size, saveFig = 'toto.jpg') : 
    ...
    for i in range(nbPoint) : 
        if mask[i] > 0.5: 
            xyA = (int(xA[i] * img_size), int(yA[i] * img_size))
            xyB = (int(xB[i] * img_size), int(yB[i] * img_size))
...
stride_net = 16
nb_feat = args.img_size / stride_net
...
for i in tqdm(range(nb_pair)) : 
    ...
    maska = np.array(Image.fromarray((infoa[2] * 255).astype(np.uint8)).resize((args.img_size, args.img_size), resample=2))
    maskb = np.array(Image.fromarray((infob[2] * 255).astype(np.uint8)).resize((args.img_size, args.img_size), resample=2))
