...
class EvalPitts:
    def __init__(self,
        ...
        self.qry_list = np.load('pitts_test_qry_list.npy')
        self.qry_utm = np.load('pitts_test_qry_utm.npy')
        self.nb_qry = len(self.qry_list) 
        
        self.db_list = np.load('pitts_test_db_list.npy')
        self.db_utm = np.load('pitts_test_db_utm.npy')
    ...
    def evalPitts(self, backbone, netEncoder):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter, description='Evaluate cross-transformer on Pitts30K')
    ...
    parser.add_argument('--qry-dir', type=str, default='/space_sdd/PlaceReco/Pittsburgh250k/queries_real/', help='query directory')

    parser.add_argument('--db-dir', type=str, default='/space_sdd/PlaceReco/Pittsburgh250k/', help='db directory')

    parser.add_argument('--score', type=str, default='top100_patchVlad.npy', help='cosine similarity with ibl feature')
    ...
    PittsEvaluator = EvalPitts(qry_dir = args.qry_dir,
                               db_dir = args.db_dir,
                               feat_dim = feat_dim)
    with torch.no_grad() : 
        PittsEvaluator.evalPitts(backbone, netEncoder)
